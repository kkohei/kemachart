...
> - [x] バージョン: v1.0 = 表示 1.0／ビルド 11（申請用）。リポジトリは次回 v1.3／ビルド 15
...
| 項目 | 場所 | 現在値 |
| --- | --- | --- |
| 表示バージョン | Xcode → General → Version（`MARKETING_VERSION`） | `1.3` |
| ビルド番号 | Xcode → General → Build（`CURRENT_PROJECT_VERSION`） | `15` |
